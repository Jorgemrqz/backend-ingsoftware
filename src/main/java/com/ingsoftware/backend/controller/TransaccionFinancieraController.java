...
import com.ingsoftware.backend.services.TransaccionFinancieraImpl;
...
public class TransaccionFinancieraController {
    ...
    @Autowired
    private TransaccionFinancieraImpl tFinancieraImpl;

    @GetMapping
    public ResponseEntity<List<TransaccionFinanciera>> getTransaccionesFinancieras() {
        return new ResponseEntity<>(this.tFinancieraImpl.getTransaccionesFinancieras(), HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<TransaccionFinanciera> getTransaccionFinanciera(@PathVariable Long id) {
        Optional<TransaccionFinanciera> transaccion = this.tFinancieraImpl.getTransaccionFinanciera(id);
    ...
    public ResponseEntity<TransaccionFinanciera> createTransaccionFinanciera(@RequestBody TransaccionFinanciera transaccion) {
        try {
           TransaccionFinanciera newTransaccion = this.tFinancieraImpl.createTransaccionFinanciera(transaccion);
    ...
    public ResponseEntity<TransaccionFinanciera> updateRubro(@RequestBody TransaccionFinanciera transaccion) {
        try {
            TransaccionFinanciera transaccionUpdated = this.tFinancieraImpl.updateTransaccionFinanciera(transaccion);
    ...
    public ResponseEntity<Void> deleteRubro(@PathVariable Long id) {
        Optional<TransaccionFinanciera> transaccion = this.tFinancieraImpl.getTransaccionFinanciera(id);
        if (transaccion.isPresent()) {       
            this.tFinancieraImpl.deleteTransaccionFinanciera(transaccion.get().getId());
